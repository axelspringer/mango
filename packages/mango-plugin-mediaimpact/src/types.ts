...
const FeaturedVideoPropertyType = new GraphQLObjectType({
    name: 'FeaturedVideoPropertyType',
    fields: () => ({
        source: {
            type: GraphQLString,
            resolve: (item) => item.source
...
const FeaturedVideoType = new GraphQLObjectType({
    ...
    fields: () => ({
        ...
        thumbnail: {
            type: FeaturedVideoPropertyType,
            resolve: (item) => {
                if (item.hasOwnProperty('rendered')) {
                    return item.rendered.thumbnail
                }
                return 0
            }
        },
        ...
        medium_large: {
            ...
            resolve: (item) => {
                if (item.hasOwnProperty('rendered')) {
                    return item.rendered.medium_large
                }
                return 0
            }
        },
